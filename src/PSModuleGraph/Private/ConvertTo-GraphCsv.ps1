function ConvertTo-GraphCsv {
    param($Graph, [switch]$IncludeUnresolved)

    $sb = [System.Text.StringBuilder]::new()
    [void]$sb.AppendLine('Source,Target,SourceName,TargetName,Kind,Path,StartLine')
    foreach ($e in $Graph.Edges) {
        [void]$sb.AppendLine((
                '{0},{1},{2},{3},{4},{5},{6}' -f
                (ConvertTo-EscapedCsvField $e.Source),
                (ConvertTo-EscapedCsvField $e.Target),
                (ConvertTo-EscapedCsvField $e.SourceName),
                (ConvertTo-EscapedCsvField $e.TargetName),
                (ConvertTo-EscapedCsvField $e.Kind),
                (ConvertTo-EscapedCsvField $e.Path),
                $e.StartLine
            ))
    }

    if ($IncludeUnresolved) {
        foreach ($u in $Graph.Unresolved) {
            [void]$sb.AppendLine((
                    '{0},{1},{2},{3},{4},{5},{6}' -f
                    (ConvertTo-EscapedCsvField $u.Source),
                    (ConvertTo-EscapedCsvField "external:$($u.TargetName)"),
                    (ConvertTo-EscapedCsvField $u.SourceName),
                    (ConvertTo-EscapedCsvField $u.TargetName),
                    'Unresolved',
                    (ConvertTo-EscapedCsvField $u.Path),
                    $u.StartLine
                ))
        }
    }

    $sb.ToString()
}
