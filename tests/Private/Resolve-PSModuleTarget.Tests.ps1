...
Describe 'Resolve-PSModuleTarget' {
    It 'resolves a directory path' {
        $t = & (Get-Module PSModuleGraph) { param($p) Resolve-PSModuleTarget -Path $p } $script:Sample
    ...
    It 'resolves a .psd1 path' {
        ...
        $t = & (Get-Module PSModuleGraph) { param($p) Resolve-PSModuleTarget -Path $p } $psd1
        $t.Name | Should-Be 'SampleModule'
        $t.Version | Should-Be ([version]'1.2.3')
    }

    It 'throws for missing path' {
        { & (Get-Module PSModuleGraph) { Resolve-PSModuleTarget -Path 'C:\no\such\module-path-xyz' } } |
            Should-Throw
    }
}
